...
class MidiPlayer {
    constructor(p5) {
        ...
        this.tsize = this.p5.height/128;

        this.g1 = this.p5.color(Math.random() * 105 + 150, Math.random() * 105 + 150, Math.random() * 105 + 150);
        this.g2 = this.p5.color(Math.random() * 105 + 150, Math.random() * 105 + 150, Math.random() * 105 + 150);
        this.bg = this.p5.lerpColor(this.g1, this.g2, 0.5);
        this.bg = this.p5.lerpColor(this.bg, this.p5.color(255), 0.7);
    ...
    loadMidis(path, onMidisLoaded) {
        ...
        let prevLength = _midiPlayer.pianoRolls.length;
        ...
        this.loadFile(path, "text", function(xhttp) {
            ...
            for(let filename in dataset) {
                ...
                _midiPlayer.loadFile(dataset[filename], "arraybuffer", function(xhttp) {
                    ...
                    if(_midiPlayer.pianoRolls.length == Object.keys(dataset).length + prevLength) {
    ...
    notes2PianoRoll(duration, notes) {
        let timeSteps = Math.ceil(duration/this.tsDuration);
        ...
        for(let note of notes) {
            let ts = Math.floor(note.time/this.tsDuration);
            let pitch = note.midi;
            let duration = Math.floor(note.duration/this.tsDuration);
    ...
    draw() {
        ...
        if (this.p5.mouseIsPressed) {
            ...
            let deltaX = this.p5.mouseX - this.p5.pmouseX;
        ...
        let s = this.tsize;
        for (let i = 0; i < this.p5.width/s; i++) {
            this.p5.rect(i*this.tsize, 0, s, s*128);
        }

        for (let j = 0; j < this.p5.height/s; j++) {
            this.p5.rect(0, j*s, s*this.p5.width/s, s);
        }

        // Draw piano roll
        if(this.pianoRollPlaying) {
            for (let i = this.ts - this.pianoRollBufferLength; i < this.ts + this.p5.width/s; i++) {
                for (let j = 0; j < this.p5.height/s; j++) {
                    if (this.pianoRollPlaying[i] && this.pianoRollPlaying[i][j] > 0) {
                        let duration = this.pianoRollPlaying[i][j];

                        if(i <= this.ts) {
                            this.p5.stroke(this.p5.lerpColor(this.g1, this.g2, 1 - i/(this.ts + this.p5.width/s)));
                        }
                        else {
                            this.p5.noStroke();
                        }

                        this.p5.fill(this.p5.lerpColor(this.g1, this.g2, i/(this.ts + this.p5.width/s)));
                        this.p5.rect((i - this.ts) * s, j*s, duration*s, s);
                    }
                }
            }
        }
        ...
        // Draw time step
        this.p5.fill(this.p5.red(this.g2)/2, this.p5.green(this.g2)/2, this.p5.blue(this.g2)/2, 150);
        this.p5.rect(0, 0, s, s*128);
    }
}
