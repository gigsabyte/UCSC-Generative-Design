...
class Race {
    ...
    getCarProgress(car) {
        return this.terrain.p5.constrain(car.getPosition().x/(this.terrain.length * this.terrain.step), 0, 1);
    }

    destroyBadCars() {
        for(let i = 0; i < this.cars.length; i++) {
            //console.log(this.cars[i].getTimeStopped() + " " + Race.maxStoppedTime);
            ...
            if(this.cars[i] && this.cars[i].getPosition().y > this.terrain.p5.height + Car.magRange[1]) {
